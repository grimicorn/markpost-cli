...
describe('index', () => {
  ...
  it('prints a subcommand group\'s usage and exits 0 for "<group> --help" without invoking the handler', async () => {
    process.argv = [...originalArgv.slice(0, 2), 'sources', '--help'];
    const { runSourcesCommand } = await import('@/commands/sources.js');
    ...
    // subcommand-position --help is intercepted here and never reaches the
